...
見出し行（テーブルヘッダー）と`Gallery`のデータ行で、列の位置・幅を**別々の方式で計算すると列がずれる。** 実際に、見出し行は`FillPortions`（比率）で列幅を決め、データ行は`Parent.TemplateWidth`に独自の%を掛けた`X`座標で位置を決める、という**方式の異なる列定義**を混在させたため、見出しの下にデータが正しく揃わない不具合が発生した。

- 見出し行を`FillPortions`で組んだら、データ行（`Gallery`のテンプレート内）の各列も**同じ`FillPortions`比率**で組む。
- `ManualLayout`で絶対配置がどうしても必要な場合は、見出し行・データ行の**両方**を同じ計算式で作る。片方だけ`FillPortions`、片方だけ独自の%計算、という混在は絶対にしない。
- 列を1つ足す・幅を変えるときは、見出し行とデータ行の**両方**を同時に直す（片方だけ直して終わらせない）。

### 親のWidthが可変（計算式）なのに、子要素のWidthを決め打ちの固定pxにしない

親コンテナの`Width`が固定値ではなく**計算式**（例: `(Parent.Width - 48 - 64) / 5`のような、画面幅に応じて変わる式）のとき、その中の子要素（ラベルなど）の`Width`を、親と無関係な**決め打ちの固定px**にすると、画面幅によっては子要素の方が実際の親の幅より広くなり、**はみ出す・切れる**ことがある。実際に、カードの幅は画面幅で変わる計算式なのに、中のラベルの`Width`が固定`160`のままになっており、画面が狭いときにテキストが欠ける不具合が発生した。

- 親の`Width`が計算式なら、子要素の`Width`も**その計算式を基準にする**（例: `Width: =Parent.Width - 32`）。
- 複数の固定`Width`列を横に並べる場合、列幅の合計＋パディングが実際に使う最小の画面幅を超えないか確認する。超える可能性がある場合は、どれか1列を`FillPortions`で伸縮可能にする。
- `Badge`など「列の幅より小さい要素」を列に配置する場合、余った分をスペーサーで埋めて、その列全体の幅が見出し行の対応する列幅と一致するようにする。
...
- [ ] 親の`Width`が計算式のとき、子要素の`Width`も決め打ちの固定pxではなく、その計算式を基準にしているか
- [ ] 固定`Width`の列を複数横に並べる場合、合計＋パディングが実際の画面幅を超えず、`Badge`等の余った分をスペーサーで埋めて見出し行の列幅と一致させているか
- [ ] コンテナの`Height`/`Width`を、丸い数字の決め打ちではなく、中身の合計サイズ＋パディング＋`LayoutGap`を実際に計算してから決めているか
