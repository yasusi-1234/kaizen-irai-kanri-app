...
**1回の依頼で複数の要素・複数の変更を追加するよう求められた場合も、全部をまとめて書いてから最後に一括で見直す、という進め方はしない。** 要素を1つ追加するごとに立ち止まり、下記チェックリストと照合してから次の要素に進む（「セクション単位で区切って自己検証する」を、1回のやり取りの中で自分自身に対して行う）。一括生成・一括レビューは、個別チェックが薄まりチェック漏れの主因になることが分かっているため、依頼が「まとめて」であっても内部の進め方は必ず1要素ずつにする。
